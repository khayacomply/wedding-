...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener("load", () => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                document.querySelector(".hero-content").style.animation = "fadeInUp 1.2s cubic-bezier(0.22, 1, 0.36, 1) forwards";
            }, 800);
        }, remainingTime);
    });
    ...
    function getSlideDistance() {
        const card = gallery.querySelector(".journey-card");
        if (!card) return 400;
    ...
    ========================================================= */
    // Select elements to reveal. Add data-delay="100" in HTML for staggering.
    const revealElements = document.querySelectorAll(
        ".intro-section, .story-heading, .program-card, .venue-content, .information-card, .journey-card"
    );
    ...
    revealElements.forEach((el, index) => {
        // Auto-stagger program cards and info cards
        if (el.classList.contains("program-card") || el.classList.contains("information-card")) {
            el.dataset.delay = (index % 3) * 150; // 0ms, 150ms, 300ms
        }
    ...
    // Check if countdown container exists in HTML
    ...
    if (countdownContainer) {
        function updateCountdown() {
            ...
            if (distance < 0) {
                countdownContainer.innerHTML = "<span>We are married! ♡</span>";
                return;
            }
            ...
            // Update HTML (Ensure you have elements with these IDs in your HTML)
